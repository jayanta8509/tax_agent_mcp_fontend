...
async function callSubClientAPI() {
    ...
    try {
        ...
        // Fallback: Enable input anyway for "Me" workflow
        console.log('Subclient API failed, falling back to "Me" workflow');
        enableUserInput();
        addMessage('agent', 'Continuing with your personal ("Me") tax filing.', null, true);
        setTimeout(() => {
            sendStartMessage();
        }, 1000);
    }
...
function displayClientSelection(clients) {
    ...
    // Add "Me" option at the beginning - FIXED reference_id
    const meOption = `
        <div class="client-box client-box-me" onclick="selectClient('${userData.client_id}', '${userData.reference_id}', '${userData.reference_id || ""}', null)">
